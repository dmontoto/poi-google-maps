...
function pgm_requirementsNotMet() {
    ...
    echo sprintf('
		...
		</div>', PGM_NAME, PGM_REQUIRED_PHP_VERSON, PGM_REQUIRED_WP_VERSION, PHP_VERSION, $wp_version
    );
}
...
if (pgm_requirementsMet()) {
    ...
    //Register Custom PostTypes and Taxonomies
    if (class_exists('Poi_Camping'))
        $pgm_camping = new Poi_Camping();
    if (class_exists('Poi_Region'))
        $pgm = new Poi_Region(array($pgm_camping->PostType));
}
else
